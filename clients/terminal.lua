...
-- Setup modem and state
peripheral.find("modem", rednet.open)
local unlocked = false

-- Function to show locked screen
local function showLockedScreen()
  term.clear()
...
-- Function to show unlocked screen
local function showUnlockedScreen()
  term.clear()
  term.setCursorPos(1, 1)
...
while true do
  if not unlocked then
    showLockedScreen()
    -- Wait until we get unlock
    while true do
      local _, _, _, _, msg = os.pullEvent("rednet_message")
      if msg == "unlock_inventory" then
        unlocked = true
        break
      end
    end
  ...
  -- Once that UI ends, we wait for a lock signal
  while true do
    local _, _, _, _, msg = os.pullEvent("rednet_message")
